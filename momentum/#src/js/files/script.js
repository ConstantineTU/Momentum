...
function setBg() {
  const exec = isWebp ? ".webp" : ".jpg";
  const adaptiveView = isWebp ? "_hd/" : "";
  const body = document.querySelector("body");
  const img = new Image();
  img.src = `https://raw.githubusercontent.com/ConstantineTU/stage1-tasks/assets/images/${adaptiveView}${getTimeOfDayForBg()}/${randomNum}${exec}`;
  img.onload = () => {
    body.style.background = `url('https://raw.githubusercontent.com/ConstantineTU/stage1-tasks/assets/images/${adaptiveView}${getTimeOfDayForBg()}/${randomNum}${exec}') center/cover no-repeat`;
    setTimeout(() => {
      loading = true;
