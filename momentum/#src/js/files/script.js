import playList from "./playList.min.js";
let isRussian = false;
let isApiFlickr;
let isApiUnsplash;
let adaptive = false;
...
function showDate() {
  ...
  date.textContent = currentDate.replace(/(^|\s)\S/g, function (a) {
...
function setBg() {
  ...
  const exec = isWebp && timeOfDay === "afternoon" ? ".webp" : ".jpg";
  const adaptiveView = isWebp ? "_hd/" : "";
  const body = document.querySelector("body");
  const img = new Image();
  img.src = `https://raw.githubusercontent.com/ConstantineTU/stage1-tasks/assets/images/${adaptiveView}${timeOfDay}/${randomNum}${exec}`;
  img.onload = () => {
    body.style.background = `url('https://raw.githubusercontent.com/ConstantineTU/stage1-tasks/assets/images/${adaptiveView}${timeOfDay}/${randomNum}${exec}') center/cover no-repeat`;
    setTimeout(() => {
      loading = true;
...
function setBgFirstLoad(webp) {
  const timeOfDay = getTimeOfDayForBg();
  const exec = webp && timeOfDay === "afternoon" ? ".webp" : ".jpg";
  const adaptiveView = webp ? "_hd/" : "";
  const body = document.querySelector("body");
  const img = new Image();
  img.src = `https://raw.githubusercontent.com/ConstantineTU/stage1-tasks/assets/images/${adaptiveView}${timeOfDay}/${randomNum}${exec}`;
  img.onload = () => {
...
if (
  localStorage.getItem("currentNameCity") &&
  localStorage.getItem("currentNameCity") !== "undefined"
) {
...
async function getWeather() {
  ...
  if (!isRussian) {
    url = `https://api.openweathermap.org/data/2.5/weather?q=${currentNameCity}&lang=en&appid=2c9de9201bfa2b88df09186fc2130c69&units=metric`;
  } else {
    url = `https://api.openweathermap.org/data/2.5/weather?q=${currentNameCity}&lang=ru&appid=2c9de9201bfa2b88df09186fc2130c69&units=metric`;
  }
    ...
    if (res.status === 200) {
      ...
      if (!isRussian) {
        ...
        if (window.matchMedia("(max-width: 768.98px)").matches) {
          ...
          wind.textContent = `Скорость ветра: ${Math.floor(
            data.wind.speed
          )} м/с`;
          humidity.textContent = `Влажность воздуха: ${Math.floor(
            data.main.humidity
          )}%`;
        }
      }
...
function playAudio() {
  ...
  if (!isPlay) {
    ...
    isPlay = true;
    if (
      localStorage.getItem("audioCurrentTime") &&
      !isNaN(localStorage.getItem("soundProgress-value"))
    ) {
    ...
    if (audio.volume === 0) {
      if (localStorage.getItem("volume-value")) {
        volumeProgress.style.background =
          localStorage.getItem("volume-progress");
...
soundProgress.addEventListener("change", function () {
  ...
  timer.textContent = `${Math.floor(audio.currentTime / 60)}:${String(
...
function changeProgressAudio() {
  ...
  if (!isChangeSoundProgress) {
    if (!isNaN(audio.duration)) {
      duration.textContent = `${Math.floor(audio.duration / 60)}:${String(
...
const albomsFlickr = {
  night:
    "https://www.flickr.com/services/rest/?method=flickr.galleries.getPhotos&api_key=87978b32968522241562a49c932bdf6b&gallery_id=185118123-72157720062587146&extras=url_h&format=json&nojsoncallback=1",
  morning:
    "https://www.flickr.com/services/rest/?method=flickr.galleries.getPhotos&api_key=87978b32968522241562a49c932bdf6b&gallery_id=185118123-72157720069530982&extras=url_h&format=json&nojsoncallback=1",
  afternoon:
    "https://www.flickr.com/services/rest/?method=flickr.galleries.getPhotos&api_key=87978b32968522241562a49c932bdf6b&gallery_id=185118123-72157720111881805&extras=url_h&format=json&nojsoncallback=1",
  evening:
    "https://www.flickr.com/services/rest/?method=flickr.galleries.getPhotos&api_key=87978b32968522241562a49c932bdf6b&gallery_id=185118123-72157720111880160&extras=url_h&format=json&nojsoncallback=1",
};
...
function getInputValue() {
  ...
  function reverseTodoList() {
    ...
    for (let i = 0; i < 50; i++) {
      if (localStorage.getItem(`NewDoneItem${i}`)) {
        ...
        const todosTrash = todoDoneListUl.querySelectorAll(
          ".todo-controlls-trash"
        );
        todosTrash.forEach(
          (todoTrash) =>
            (todoTrash.onclick = function () {
  ...
  function calculateTodos() {
    const activeCurrentyTodo = document.querySelector(
      ".todo-top_active-currenty"
    );
    ...
    if (activeCurrenty) {
      ...
      activeCurrentyTodo.textContent = " ";
    }
    if (doneCurrenty) {
      doneCurrentyTodo.textContent = doneCurrenty;
    } else {
      doneCurrentyTodo.textContent = " ";
    }
  }
  ...
  function deleteDoneTodoItems(target) {
    for (let i = 0; i < todoDoneItems.length; i++) {
      if (
        todoDoneItems[i].classList.contains("done") &&
        target === todoDoneItems[i]
      ) {
        localStorage.removeItem(`NewDoneItem${i}`);
        todoDoneItems[i].remove();
        isDeleteTodo = true;
    ...
    doneCurrenty = document
      .querySelector(".todo-done-list")
...
function addLink() {
  ...
  function loadLinksItems() {
    ...
    for (let i = 0; i < 50; i++) {
      if (
        localStorage.getItem(`linkItemName${i}`) &&
        localStorage.getItem(`linkItemName${i}`) !== "undefined"
      ) {
        createLink(
          localStorage.getItem(`linkItemName${i}`),
          localStorage.getItem(`linkItemUrl${i}`)
        );
      }
    }
  }
  ...
  function validAddLink(keyPressed) {
    ...
    if (linksItems.length < 29 || !isBlockedAddLink) {
      ...
      if (isRussian) {
        error.textContent =
          "Пожалуйста удалите закладку, прежде чем добавлять новую";
      } else {
